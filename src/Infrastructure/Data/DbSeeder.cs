...
public static class DbSeeder
{
    public static async Task MigrateAndSeedAsync(this IServiceProvider services, CancellationToken cancellationToken = default)
    {
        ...
        double ApproxDistance(string district)
        {
            ...
            {
                if (existingToCities.Contains(district)) continue;
        ...
        decimal ComputePrice(double km, string busName)
        {
            var basePerKm = 3.5m;
            var factor = 1.0m;
            if (busName.Contains("AC Sleeper", StringComparison.OrdinalIgnoreCase)) factor = 5.0m / 3.5m;
            else if (busName.Contains("AC Business", StringComparison.OrdinalIgnoreCase)) factor = 4.2m / 3.5m;
